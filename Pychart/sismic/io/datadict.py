...
def _import_state_from_dict(state_d: Mapping[str, Any]) -> StateMixin:
    ...
    if stype == 'final':
        state = FinalState(name,position, on_entry=on_entry, on_exit=on_exit)
    elif stype == 'shallow history':
        state = ShallowHistoryState(name,position, on_entry=on_entry, on_exit=on_exit,
                                    memory=state_d.get('memory', None))
    elif stype == 'deep history':
        state = DeepHistoryState(name,position, on_entry=on_entry, on_exit=on_exit,
                                 memory=state_d.get('memory', None))
    elif stype is None:
        ...
        if substates and parallel_substates:
            ...
            state = CompoundState(name,position,initial=state_d.get('initial', None),
                                  on_entry=on_entry, on_exit=on_exit)
        elif parallel_substates:
            state = OrthogonalState(name,position, on_entry=on_entry, on_exit=on_exit)
        else:
            state = BasicState(name,position, on_entry=on_entry, on_exit=on_exit)
    else:
